...
class ViewSolicitudPDF extends CdtPDFPrint{
    ...
    function integrante() {
        $ds_tipoSTR = ($this->getDs_tipo()=='CAMBIODEDHS')?'CAMBIO EN LA DED. HORARIA':(($this->getDs_tipo()=='CAMBIOTIPO')?'CAMBIO DE TIPO':$this->getDs_tipo());
        $this->SetFillColor(200,200,200);
        $this->SetFont ( 'Arial', 'B', 10 );
        ...
        $this->Cell ( 30, 6, "");
        $this->SetFont ( 'Arial', 'B', 10 );
        $this->Cell ( 65, 6, $this->encodeCharacters("CategorÃ­a de Docente Investigador:"));
        $this->SetFont ( 'Arial', '', 10 );
        ...
        if (($this->getDs_tipo() == 'ALTA')||($this->getDs_tipo() == 'CAMBIO')){
            ...
            if ($this->getDs_titulogrado()=='') {
                ...
                $this->Cell ( 25, 6, "Estudiante:");
                $this->SetFont ( 'Arial', '', 10 );
                $bl_estudiante = ($this->getNu_materias()>0)?'SI':'NO';
                $this->Cell ( 10, 6, stripslashes($bl_estudiante), 'LTBR',0,'L',1);
                $this->SetFont ( 'Arial', 'B', 10 );
                $this->Cell ( 20, 6, "Carrera:");
                $this->SetFont ( 'Arial', '', 10 );
                $this->Cell ( 40, 6, $this->encodeCharacters($this->getDs_carrera()), 'LTBR',0,'L',1);
                ...
                $this->Cell ( 40, 6, $this->encodeCharacters("TÃ­tulo de Grado:"));
                $this->SetFont ( 'Arial', '', 10 );
                $this->MultiCell( 145, 4, $this->encodeCharacters($this->getDs_titulogrado()), 'LTBR','L');
                $this->ln(3);
                $this->SetFont ( 'Arial', 'B', 10 );
                $this->Cell ( 40, 6, $this->encodeCharacters("TÃ­tulo de posgrado:"));
                $this->SetFont ( 'Arial', '', 10 );
        ...
        $this->Cell ( 20, 6, "");
        $this->SetFont ( 'Arial', 'B', 10 );
        $this->Cell ( 25, 6, $this->encodeCharacters("DedicaciÃ³n:"));
        $this->SetFont ( 'Arial', '', 10 );
        $this->Cell ( 20, 6, $this->encodeCharacters($this->getDs_deddoc()), 'LTBR',0,'L',1);
        if (($this->getDs_tipo() == 'CAMBIO')||($this->getDs_tipo() == 'CAMBIODEDHS')||($this->getDs_tipo() == 'CAMBIOTIPO')) {
            $this->ln(8);
            $this->SetFont ( 'Arial', 'B', 10 );
            $this->Cell ( 80, 6, $this->encodeCharacters("Fecha ObtenciÃ³n Cargo:"));
            $this->SetFont ( 'Arial', '', 10 );
        ...
        $this->SetFont ( 'Arial', 'B', 10 );
        $this->Cell ( 22, 6, $this->encodeCharacters("InstituciÃ³n:"));
        $this->SetFont ( 'Arial', '', 10 );
        ...
        $this->ln(8);
        if (($this->getDs_tipo() == 'ALTA')||($this->getDs_tipo() == 'CAMBIO')||($this->getDs_tipo() == 'CAMBIOTIPO')){
            if ($this->getBl_becaEstimulo()) {
                $this->SetFont ( 'Arial', 'B', 10 );
                $this->Cell ( 80, 6, $this->encodeCharacters("Beca de EstÃ­mulo a las Vocaciones CientÃ­ficas:"));
                $this->SetFont ( 'Arial', '', 10 );
        ...
        $this->ln(3);
        if (($this->getDs_tipo() == 'ALTA')||($this->getDs_tipo() == 'CAMBIO')||($this->getDs_tipo() == 'CAMBIODEDHS')||($this->getDs_tipo() == 'CAMBIOTIPO')){

            if (($this->getDs_tipo() != 'CAMBIODEDHS')&&($this->getDs_tipo() != 'CAMBIOTIPO')) {
            ...
            if (($this->getDs_tipo() != 'CAMBIODEDHS')&&($this->getDs_tipo() != 'CAMBIOTIPO')) {
                ...
                if ($this->getDs_tipo() != 'CAMBIOTIPO') {
                    $this->Cell ( 52, 6, "Hs. dedicadas actualmente:");
                    $this->SetFont ( 'Arial', '', 10 );
                    $this->Cell ( 10, 6, $this->getNu_horasinvAnt(), 'LTBR',0,'L',1);
                }
                else{
                    $this->Cell ( 48, 6, "Hs. dedicadas actualmente:");
                    $this->SetFont ( 'Arial', '', 10 );
                    $this->Cell ( 6, 6, $this->getNu_horasinvAnt(), 'LTBR',0,'L',1);
                }
            }
            ...
            if ($this->getDs_tipo() != 'CAMBIODEDHS') {
                if ($this->getDs_tipo() != 'CAMBIOTIPO') {
                    ...
                    $this->Cell ( 2, 6, "");
                    $this->Cell ( 28, 6, "Hs. solicitadas:");
                    $this->SetFont ( 'Arial', '', 10 );
                    $this->Cell ( 6, 6, $this->getNu_horasinv(), 'LTBR',0,'L',1);
                    $this->SetFont ( 'Arial', 'B', 10 );
                    $this->Cell ( 38, 6, "Tipo de Investigador:");
                    $this->SetFont ( 'Arial', '', 10 );
                    $this->Cell ( 57, 6, $this->encodeCharacters($this->getDs_tipoinvestigador()), 'LTBR',0,'L',1);
                }
            ...
            $this->row(array($this->encodeCharacters('CÃ³digo'),$this->encodeCharacters('TÃ­tulo'),'Director','Tipo',$this->encodeCharacters('PerÃ­odo'),'Hs. x Sem.'));
            ...
            if (($this->getDs_tipo() == 'CAMBIODEDHS')||($this->getDs_tipo() == 'CAMBIOTIPO')) {
                if ($this->getDs_reduccionHS()) {
                    ...
                    $this->ln(8);
                    $this->SetFont ( 'Arial', 'B', 10 );
                    $this->MultiCell( 185, 4, $this->encodeCharacters('En el caso de ser una reducciÃ³n horaria, especificar las consecuencias que la misma tendrÃ¡Â¡ en el desarrollo del proyecto'), '','L');
                    $this->ln(3);
                    $this->SetFont ( 'Arial', '', 8 );
                    $this->MultiCell( 185, 4, $this->encodeCharacters($this->getDs_reduccionHS()), 'LTBR','L');
                    $this->ln(3);
                    $this->SetFont ( 'Arial', 'B', 10 );
                    $this->MultiCell( 185, 4, $this->encodeCharacters('Considerando la reducciÃ³n, el proyecto cumple con las pautas fijadas en la AcreditaciÃ³n: La suma de dedicaciones horarias de los miembros del proyecto es igual o mayor a ').$minhstotales.' hs. semanales', 'LTBR','L');
            ...
            //$this->ln(3);
            $this->MultiCell( 185, 4, $this->encodeCharacters('Se cumple con las pautas fijadas en la AcreditaciÃ³n'), 'LTBR','L');
            $this->ln(3);
        }
        $this->ln(8);
        $this->SetFont ( 'Arial', 'B', 10 );
    ...
    function firma() {
        ...
        $this->Cell ( 30, 8);
        $this->Cell ( 60, 8, $this->encodeCharacters('Firma y AclaraciÃ³n'), '', 0, 'C');
        $this->ln(1);
        $this->Cell ( 185, 8, '', 'B');
        $this->ln(10);
        $this->Cell ( 10, 8);
        ...
        $this->ln(10);
        $this->SetFont ( 'Arial', 'B', 10 );
        $this->Cell ( 185, 6, $this->encodeCharacters("La informaciÃ³n detallada en esta solicitud tiene carÃ¡cter de DECLARACION JURADA."),0,0,'',1);
    ...
    function Header(){
        ...
        if (($this->getEstado_oid()==CYT_ESTADO_INTEGRANTE_ALTA_CREADA)||($this->getEstado_oid()==CYT_ESTADO_INTEGRANTE_BAJA_CREADA)||($this->getEstado_oid()==CYT_ESTADO_INTEGRANTE_CAMBIO_CREADO)||($this->getEstado_oid()==CYT_ESTADO_INTEGRANTE_CAMBIO_HS_CREADO)||($this->getEstado_oid()==CYT_ESTADO_INTEGRANTE_CAMBIO_TIPO_CREADO)) {
     ...
     * @return string
     */
    public function getNu_totalMat(): string
    {
        return $this->nu_totalMat;
    }

    /**
     * @param string $nu_totalMat
     */
    public function setNu_totalMat(string $nu_totalMat)
    {
     ...
     * @return string
     */
    public function getDs_carrera(): string
    {
        return $this->ds_carrera;
    }

    /**
     * @param string $ds_carrera
     */
    public function setDs_carrera(string $ds_carrera)
    {
